...
public:
  ...
  IFieldInitializer *clone() const { return new DefaultFieldInitializer(); }
...
public:
  ...
  void initialize(FieldBitmap &field);
  IFieldInitializer *clone() const { return new RandomObstaclesFI(*this); }

  ~RandomObstaclesFI() = default;

private:
  void _finalize_field(FieldBitmap &field);
  void _find_obstacle_place(const Obstacle &obstacle, const FieldBitmap &field,
                            int &x, int &y, bool exhaustive);
  int _insert_obstacle(const Obstacle &obstacle, FieldBitmap &field, int x,
                       int y, int max_len);
};
