...
"Крестики-нолики" до 5 в ряд на большом поле с препятствиями. Студенту необходимо реализовать
...
- Алгоритм должен быть применим для игры со следующими параметрами: размер поля 20 на 20, длина выигрышной последовательности 5, `playable_part = 0.75`, `gap = 1`, `max_obstacle_len = 50`. Описание данных параметров представлено в [разделе ниже](#anchor)

- Зафиксированные выше параметры должны быть использованы при проведении тестов и турнира
...
- Алгоритм должен быть достаточно сильным. Формально это означает, что процент побед над игроком базового уровня сложности без учета ничьих должен быть больше 50%, над игроком повышенного уровня сложности - больше 25%. Данные игроки предоставляются в виде подключаемой библиотеки с закрытым исходным кодом, которая подключается к вашему проекту
...
поле состоит из `M * N` клеток, некоторые из которых помечены как недоступные для совершения хода (будем говорить, что в них расположены препятствия).

Каждый игрок по очереди ставит свой символ в свободную клетку этого поля. Занятыми считаются клетки, которые либо содержат препятствие `#`, либо символ одного из игроков. Первый
...
Игровое состояние определено в файле `core/state.hpp` классом `ttt::game::State`. Игровое состояние может быть создано с настройками игры, которые определяются структурой `ttt::game::State::Opts`. Структура содержит настройки размера поля, параметра `L` и максимального количества ходов. Для настройки режима генерации препятствий можно воспользоваться встроенным классом `ttt::game::RandomObstaclesFI`. 

<a id="anchor"></a>`ttt::game::RandomObstaclesFI` реализует паттерн "стратегия", и передается по указателю в класс `ttt::game::Game`. Для генерации препятствий имеется 3 параметра: `playable_part`, `max_obstacle_len`, `gap`. `playable_part` - это доля доступных для игры клеток от 0 до 1. Реально свободных клеток на поле после генерации будет не менее `playable_part`, так как используется жадный алгоритм размещения препятствий. `max_obstacle_len` - это максимальная протяженность препятствия. При значении параметра 0 будут генерироваться только отдельные клетки, при больших значениях будут использоваться препятствия, в которые входит от 0 до `max_obstacle_len + 1` клеток. `gap` - это гарантированный зазор (количество клеток) между отдельными препятствиями. 

В методе `State::process_move` реализована логика изменения состояния в зависимости от
...
Алгоритм вашего бота в отчете должен быть достаточно подробным, но при этом простым для понимания без уточняющих вопросов со стороны преподавателя. В качестве примера опишем алгоритм заполнения поля препятствиями, реализованный в `ttt::game::RandomObstaclesFI`:

Вход: `0 <= playable_part <= 1` - часть клеток, доступная для игры; `max_obstacle_len >= 0` - максимальная длина препятствия; `gap >= 0` - зазор между отдельными препятствиями; пустое поле размером `M` на `N`.

Выход: поле, случайным образом заполненное препятствиями.

Примечание: будем обозначать поле _достаточно заполненным_, если пункт алгоритма 4 завершился неудачей `sqrt(M * N)` раз подряд. Высказывания "поле _достаточно заполненное_" и "поле _достаточно пустое_" являются противоположными.

1. Сгенерировать случайное число `obstacle_len` от 0 до `max_obstacle_len`. Сгенерировать случайную последовательность длиной `obstacle_len` направлений шагов (`вверх`, `вниз`, `влево`, `вправо`), при этом любые два последовательных шага не являются противоположными (или шагами в обратную сторону).

2. Если поле _достаточно пустое_, то сгенерировать случайную позицию в пределах поля, начиная с которой предполагается искать координаты для расположения препятствия. Если поле _достаточно заполненное_, то поиск позиции для размещения препятствия начинается с клетки (0, 0).

3. Начиная с выбранной в пункте 2 позиции, последовательно перебрать все клетки поля и попробовать вставить в них препятствие. Форма препятствия определяется последовательностью шагов, сгенерированных в пункте 1. Вставка считается успешной, если все клетки препятствия располагаются в пустых доступных для использования клетках. Препятствие размещается с учетом нижней границы числа доступных для игры клеток `M * N * playable_part`, лишняя часть препятствия обрубается.

4. Если получилось вставить препятствие, то пометить клетки вокруг него в радиусе `gap` как недоступные для размещения других препятствий. Если не получилось найти позицию для препятствия, то повторить шаг 1, но при этом если шаг 3 не завершился удачей уже `sqrt(M * N)` раз подряд, то пометить поле как _достаточно заполненное_. Если после и так _достаточно заполненное_, то остановить процесс генерации, выйти из алгоритма.

5. Если игровых клеток на поле стало `M * N * playable_part`, то остановить процесс генерации, выйти из алгоритма. Если свободных клеток больше, перейти к шагу 1.
